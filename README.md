...
本项目使用 DTX (DocStrip) 格式，所有文件都从 `nsfc.dtx` 生成。
...
详细功能说明和使用方法请参考 `nsfc.dtx` 编译生成的文档（运行 `make doc`）。
...
- `nsfc.dtx` - 源代码文件（包含所有代码和文档）
- `nsfc.sty` - 样式包（从 `nsfc.dtx` 生成）
- `nsfc.ins` - 安装脚本（从 `nsfc.dtx` 生成）
- `nsfc-main.tex` - 示例主文档（从 `nsfc.dtx` 生成）
- `nsfc-refs.bib` - 示例参考文献（从 `nsfc.dtx` 生成）
- `Makefile` - 编译自动化脚本
- `.latexmkrc` - latexmk 配置文件

**注意**：所有生成的文件（`.sty`, `.ins`, `.tex`, `.bib`, `.pdf` 等）都在 `.gitignore` 中，不会被版本控制。只跟踪源文件 `nsfc.dtx`。
